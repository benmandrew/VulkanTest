...
void createImage(Device* device, uint32_t width, uint32_t height,
  ...
  if (vkCreateImage(device->logical, &imageInfo, nullptr, &image) !=
      VK_SUCCESS) {
...
VkImageView createImageView(Device* device, VkImage image, VkFormat format,
  ...
  if (vkCreateImageView(device->logical, &viewInfo, nullptr, &imageView) !=
      VK_SUCCESS) {
...
void createBuffer(Device* device, VkDeviceSize size, VkBufferUsageFlags usage,
  ...
  if (vkCreateBuffer(device->logical, &bufferInfo, nullptr, &buffer) !=
      VK_SUCCESS) {
  ...
  allocInfo.memoryTypeIndex =
      findMemoryType(device, memRequirements.memoryTypeBits, properties);
  if (vkAllocateMemory(device->logical, &allocInfo, nullptr, &bufferMemory) !=
      VK_SUCCESS) {
...
VkFormat findSupportedFormat(Device* device,
...
QueueFamilyIndices findQueueFamilies(Instance* instance,
  ...
  for (const auto& queueFamily : queueFamilies) {
	...
	vkGetPhysicalDeviceSurfaceSupportKHR(device, i, instance->surface->surface,
	                                     &presentSupport);
	if (presentSupport) {
...
SwapChainSupportDetails querySwapChainSupport(Instance* instance,
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, instance->surface->surface,
                                            &details.capabilities);
  uint32_t formatCount;
  vkGetPhysicalDeviceSurfaceFormatsKHR(device, instance->surface->surface,
                                       &formatCount, nullptr);
  if (formatCount != 0) {
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(device, instance->surface->surface,
                                            &presentModeCount, nullptr);
  if (presentModeCount != 0) {
	details.presentModes.resize(presentModeCount);
	vkGetPhysicalDeviceSurfacePresentModesKHR(
	    device, instance->surface->surface, &presentModeCount,
...
VkShaderModule createShaderModule(Device* device,
  ...
  if (vkCreateShaderModule(device->logical, &createInfo, nullptr,
